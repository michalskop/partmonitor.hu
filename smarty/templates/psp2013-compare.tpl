        ...
        <tbody>
          {foreach $questions as $question}
            ...
            <tr {if isset($user.weight.$q_id)}class="compare-important"{/if}>
	          ...
	          <td class="compare-table-td-2">{if isset($user.vote.$q_id)}{if $user.vote.$q_id == 1}Áno{elseif $user.vote.$q_id == -1}Nie{else}-{/if}{else}-{/if}</td>
      		  {foreach $mps as $key=>$mp}	  
      		  <td class="compare-{if isset($user.vote.$q_id)}{if ($mp->vote->$q_id * $user.vote.$q_id) >= .33}agree{elseif ($mp->vote->$q_id * $user.vote.$q_id) < -.33}disagree{else}neutral{/if}{/if}">{if isset($mp->vote->$q_id)}{if $mp->vote->$q_id >= .33}Áno{elseif $mp->vote->$q_id >= -.34}-{else}Nie{/if}{else}={/if}
