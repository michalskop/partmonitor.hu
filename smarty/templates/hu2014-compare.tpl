      ...
      <table class="compare-table">
        <thead>
          <tr>
            ...
            <th class="compare-table-td-n" style="text-align: center;" title="{$mp->short_name}">
            {$mp->match}&nbsp;%<br/>
            {$mp->short_name|truncate:15:"...":true}
            </th>
            {/foreach}
          </tr>
        </thead>
        <tbody>
          {foreach $questions as $question}
            ...
            <tr {if isset($user.weight.$q_id)}class="compare-important"{/if}>
	          ...
	          <td class="compare-table-td-2">{if isset($user.vote.$q_id)}{if $user.vote.$q_id == 1}Igen{elseif $user.vote.$q_id == -1}Nem{else}-{/if}{else}-{/if}</td>
      		  {foreach $mps as $key=>$mp}	  
      		  <td class="compare-{if isset($user.vote.$q_id)}{if ($mp->vote->$q_id * $user.vote.$q_id) >= .33}agree{elseif ($mp->vote->$q_id * $user.vote.$q_id) < -.33}disagree{else}neutral{/if}{/if}">{if isset($mp->vote->$q_id)}{if $mp->vote->$q_id >= .33}Igen{elseif $mp->vote->$q_id >= -.34}-{else}Nem{/if}{else}={/if}
      		  {if (isset($details[$mp->friendly_name][$q_id]))}{include "hu2014-compare-popup.tpl"}{/if}
      		  </td>
      		  {/foreach}
        	</tr>
          {/foreach}
